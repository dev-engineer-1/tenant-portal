...
export function calculateLeaseBalanceSummary(input: LeaseBalanceSummaryInput): number {
  ...
  if (totalRent <= 0 || paymentsMade.some(payment => payment < 0) || lateFees.some(fee => fee < 0)) {
  ...
  const totalLateFees = lateFees.reduce((acc, fee) => acc + fee, 0);

  return totalRent - totalPayments + totalLateFees;
}
